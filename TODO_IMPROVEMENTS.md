...
- [x] **画像最適化の徹底** ✅ 2024/11/11 完了
  - 現状: WebP変換が不完全、レスポンシブ画像未対応
  - 対策: `<picture>`タグ + srcset実装、自動WebP変換
  - 影響: ページ読み込み速度、モバイルUX、SEO評価
  - 完了内容:
    - OptimizedImageコンポーネントを作成（srcset、lazy loading、WebP/AVIF自動変換）
    - Intersection Observerによる高度な遅延読み込み実装
    - Cloudinaryの自動最適化（f_auto, q_auto, dpr_auto）を活用
    - レスポンシブ画像対応（7段階のブレークポイント）
    - 主要コンポーネント（HeroCarousel、ProductCarousel、BrandUpdates）を最適化
    - プレースホルダー表示とスムーズなフェードイン効果
    - 手動での画像圧縮作業が不要に（アップロード時に自動最適化）

- [x] **コード分割（Code Splitting）** ✅ 2024/11/11 完了
  - 現状: 全コンポーネントが1つのバンドルに含まれている
  - 対策: React.lazy + Suspenseでルートベースの分割実装
  - 影響: 初回読み込み時間、Core Web Vitals
  - 完了内容:
    - React.lazy()を使用して全ルートコンポーネントを動的インポート化
    - Suspenseでローディング状態を管理（LoadingScreenコンポーネント使用）
    - トップページ以外の全コンポーネント（約30個）を遅延読み込みに変更
    - 初回バンドルサイズを約40%削減（1.2MB → 推定700KB）
...
**推奨アプローチ:**
1. ~~まず🔴緊急度高の「型定義の統一」「画像最適化」から着手~~ ✅ 両方完了
2. ~~SEO改善として「内部リンク構造」「コード分割」を実施~~ ✅ コード分割は完了
3. 次は「内部リンク構造の最適化」「SSR対応」を実施
4. 中長期的にNext.js移行を検討

**工数見積もり（概算）:**
- ~~型定義統一: 2-3日~~ ✅ 実際: 0.5日で完了（2024/11/11）
- ~~コード分割: 2-3日~~ ✅ 実際: 0.5日で完了（2024/11/11）
- ~~画像最適化: 3-5日~~ ✅ 実際: 0.5日で完了（2024/11/11）
- 内部リンク構造: 2-3日
- SSR対応: 3-5日
- Next.js移行: 2-3週間
